...
import { Star, CheckCircle, Ticket, Users, Shield, Sparkles } from 'lucide-react';
...
const mentors = [
  { name: 'MKV', role: 'Legendary Teacher • AlgoUniversity', highlight: 'Hosting the secret-topic live class', tint: 'from-emerald-500/20 to-teal-500/10' },
  { name: 'Aisha Khan', role: 'Staff Engineer • HelioStack', highlight: 'Systems design & backend performance', tint: 'from-fuchsia-500/20 to-purple-500/10' },
  { name: 'Vedant Rao', role: 'ML Lead • NimbusAI', highlight: 'AI problem solving & evaluations', tint: 'from-sky-500/20 to-cyan-500/10' },
  { name: 'Ritika Shah', role: 'Product Engineer • CloudNest', highlight: 'Front-end craftsmanship & DX', tint: 'from-amber-500/20 to-orange-500/10' },
];
...
const textTestimonials = [
  { name: 'Vikram', role: 'Placed via AU', quote: 'The format is brilliant. You learn, compete, and recruiters already know what you can do.' },
  { name: 'Sneha', role: 'Contest Top 10', quote: 'MKV’s session was electric. The contest aligned perfectly and the job pairing felt on point.' },
  { name: 'Kabir', role: 'Backend Engineer', quote: 'Best in-person tech event I’ve attended. Seamless and truly merit-based.' },
];

export default function ExperienceAndCTA() {
  ...
  return (
    <section id="cta" className="relative mx-auto max-w-7xl px-6 py-20">
      <div className="mb-10 flex flex-col items-start justify-between gap-4 md:flex-row md:items-end">
        <div>
          <h2 className="text-3xl font-semibold tracking-tight md:text-4xl">Mentors, stories, and your ticket</h2>
          <p className="mt-2 max-w-2xl text-white/70">Real mentors, real outcomes, and a limited-time early bird to lock your seat.</p>
        </div>
        <div className="flex items-center gap-3 text-white/70">
          <div className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-3 py-1 text-xs">
            <Shield size={14} className="text-emerald-300" /> Trust-first, merit-based matching
          </div>
          <div className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-3 py-1 text-xs">
            <Users size={14} className="text-sky-300" /> 500+ past attendees
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {mentors.map((m, i) => (
          <div key={i} className="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03]">
            <div className={`h-40 w-full bg-gradient-to-tr ${m.tint} flex items-center justify-center`}>
              <div className="text-center">
                <div className="mx-auto mb-2 h-12 w-12 rounded-full bg-white/20 ring-1 ring-white/30" />
                <p className="text-xs text-white/70">Portrait</p>
              </div>
            </div>
            <div className="p-5">
      ...
      <div className="mt-16 overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-emerald-500/10 via-teal-500/5 to-transparent p-6">
        <div className="flex flex-col items-start justify-between gap-6 md:flex-row md:items-center">
          <div>
            ...
            <div className="mt-2 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-3 py-1 text-xs text-white/80">
              <Sparkles size={14} className="text-emerald-300" /> Learn → Compete → Get matched
            </div>
          </div>
